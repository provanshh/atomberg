...
import { Bell, ChevronDown, LayoutDashboard, Target, CheckCircle2, Users, BarChart3, Shield, ScrollText, Settings, LogOut, Search, Command, Sparkles } from "lucide-react";
...
const NAV: Record<string, { label: string; to: string; icon: any }[]> = {
  employee: [
...
export function AppShell() {
  const { currentUser, logout, notifications, markAllNotificationsRead, users, setCurrentUser } = useApp();
  ...
  const nav = NAV[role];
  const myNotifs = useMemo(() => notifications.filter((n) => n.recipientId === currentUser?.id), [notifications, currentUser]);
  ...
  return (
    <div className="h-screen flex bg-background bg-mesh overflow-hidden">
      ...
      <aside className="hidden lg:flex w-64 shrink-0 flex-col border-r border-sidebar-border bg-sidebar relative overflow-hidden">
        <div aria-hidden className="pointer-events-none absolute -top-20 -left-10 h-56 w-56 rounded-full bg-primary/20 blur-3xl dark:bg-primary/30" />
        <div aria-hidden className="pointer-events-none absolute bottom-10 -right-10 h-48 w-48 rounded-full bg-accent/20 blur-3xl dark:bg-accent/25" />
        ...
        <div className="relative px-5 h-16 flex items-center gap-2 border-b border-sidebar-border">
          ...
          <div>
            ...
            <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Enterprise OKRs</div>
          </div>
        </div>
            ...
            {nav.map((item) => {
              ...
              return (
                <Link key={item.to} to={item.to} className={cn(
                  ...
                  {active && <span className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-0.5 rounded-r bg-gradient-primary shadow-glow" />}
                  <Icon className={cn("h-4 w-4 transition-colors", active && "text-primary")} />
                  <span>{item.label}</span>
                  {item.to === "/approvals" && role === "manager" && <span className="ml-auto text-[10px] rounded-md px-1.5 py-0.5 bg-gradient-primary text-primary-foreground">New</span>}
                </Link>
              );
            })}
          </nav>

          <div className="mt-6 px-2 pb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Quick links</div>
          <nav className="space-y-0.5">
            <Link to="/notifications" className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-sidebar-accent/60 transition-colors">
              <Bell className="h-4 w-4" /><span>Inbox</span>
              {unread > 0 && <span className="ml-auto text-[10px] rounded-full px-1.5 bg-destructive text-destructive-foreground">{unread}</span>}
            </Link>
            <Link to="/cycles" className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-muted-foreground hover:text-foreground hover:bg-sidebar-accent/60 transition-colors">
              <Settings className="h-4 w-4" /><span>Cycles</span>
            </Link>
          </nav>
        </div>
        ...
        <header className="h-16 border-b border-border bg-card/70 backdrop-blur-xl sticky top-0 z-30">
          <div className="h-full px-4 lg:px-6 flex items-center gap-3">
            <div className="text-sm text-muted-foreground hidden md:flex items-center gap-1.5">
              ...
              <span className="text-foreground font-medium capitalize">{path.split("/").filter(Boolean).join(" / ") || "dashboard"}</span>
            </div>
            ...
            <Popover>
              ...
              <PopoverContent align="end" className="w-80 p-0">
                ...
                <div className="max-h-80 overflow-auto">
                  {myNotifs.length === 0 && <div className="p-6 text-center text-sm text-muted-foreground">You're all caught up.</div>}
                  {myNotifs.map((n) => (
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-2 pl-1 pr-2 py-1 rounded-full hover:bg-muted transition-colors">
                  <Avatar className="h-7 w-7"><AvatarFallback className="text-xs bg-gradient-primary text-primary-foreground">{initials(currentUser.name)}</AvatarFallback></Avatar>
                  <div className="hidden md:block text-left">
                    <div className="text-xs font-semibold leading-none">{currentUser.name}</div>
                    <div className="text-[10px] text-muted-foreground capitalize mt-0.5">{currentUser.role}</div>
                  </div>
              ...
              <DropdownMenuContent align="end" className="w-64">
                <DropdownMenuLabel>
                  ...
                  <div className="mt-1.5"><Badge variant="secondary" className="text-[10px] capitalize">{currentUser.role}</Badge></div>
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
                <DropdownMenuLabel className="text-[10px] uppercase tracking-wider text-muted-foreground">Switch demo persona</DropdownMenuLabel>
                {users.filter(u => ["u-emp","u-mgr","u-admin"].includes(u.id)).map((u) => (
                  <DropdownMenuItem key={u.id} onClick={() => setCurrentUser(u.id)} className="text-sm">
                    <Avatar className="h-6 w-6 mr-2"><AvatarFallback className="text-[10px]">{initials(u.name)}</AvatarFallback></Avatar>
