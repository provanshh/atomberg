...
interface AppContextValue extends AppState {
  ...
  logout: () => void;
  signup: (data: { name: string; email: string; role: User["role"]; department: string; designation: string; password?: string }) => Promise<User>;
  setCurrentUser: (id: string) => void;
  addGoal: (g: Omit<Goal, "id" | "createdAt" | "updatedAt" | "progressPercentage" | "progressStatus">) => Goal;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const syncData = useCallback(async (userId: string, role: string) => {
    try {
      ...
      const rawProfile = await api.get<any>("/users/profile");
      const loggedInUser = mapUser(rawProfile);
      
      // 2. Fetch goals
      const rawMyGoals = await api.get<any[]>("/goals/my-goals");
      let allGoals = rawMyGoals.map(mapGoal);

      if (role === "manager" || role === "admin") {
        try {
          const rawTeamGoals = await api.get<any[]>("/goals/team-goals");
      ...
      // 3. Fetch checkins
      const rawMyCheckins = await api.get<any[]>("/checkins/my-checkins");
      let allCheckIns = rawMyCheckins.map(mapCheckIn);

      if (role === "manager" || role === "admin") {
        try {
          const rawTeamCheckins = await api.get<any[]>("/checkins/team");
      ...
      if (role === "admin") {
        try {
          const rawAudit = await api.get<any[]>("/audit");
      ...
      try {
        if (role === "admin") {
          const rawUsers = await api.get<any[]>("/users/all");
          usersList = rawUsers.map(mapUser);
        } else if (role === "manager") {
          const rawTeam = await api.get<any[]>("/users/team");
  ...
  const signup = useCallback(async (data: { name: string; email: string; role: User["role"]; department: string; designation: string; password?: string }) => {
    try {
      const payload = {
        name: data.name,
        email: data.email,
      ...
      // 1. Sign up
      const rawUser = await api.post<any>("/auth/signup", payload);
      const user = mapUser(rawUser);
  ...
  const addGoal = useCallback((g: Omit<Goal, "id" | "createdAt" | "updatedAt" | "progressPercentage" | "progressStatus">) => {
    ...
    pushAudit({ userId: g.employeeId, actionType: "CREATE", moduleName: "Goals", newValue: g.title });

    // Background server call
    (async () => {
      try {
        const payload = mapGoalForBackend(newGoal);
        const rawRes = await api.post<any>("/goals/create", payload);
  ...
  const updateGoal = useCallback((id: string, patch: Partial<Goal>) => {
    ...
    (async () => {
      try {
        ...
        const rawRes = await api.patch<any>(`/goals/update/${id}`, payload);
  ...
  const submitGoals = useCallback((employeeId: string) => {
    // Optimistic update
    setState((s) => ({
      ...s,
      goals: s.goals.map((g) => (g.employeeId === employeeId && g.status === "draft" ? { ...g, status: "submitted" } : g)),
    }));
    ...
    (async () => {
      try {
        ...
        const raw = await api.get<any[]>("/goals/my-goals");
  ...
  const approveGoal = useCallback((id: string, comment?: string) => {
    // Optimistic update
    setState((s) => ({
      ...s,
      goals: s.goals.map((g) => (g.id === id ? { ...g, status: "approved", isLocked: true, managerComment: comment } : g)),
    }));
    pushAudit({ userId: state.currentUserId ?? "system", actionType: "APPROVE", moduleName: "Goals", newValue: "approved" });
  ...
  const rejectGoal = useCallback((id: string, comment: string) => {
    // Optimistic update
    setState((s) => ({
      ...s,
      goals: s.goals.map((g) => (g.id === id ? { ...g, status: "rejected", managerComment: comment } : g)),
    }));
    pushAudit({ userId: state.currentUserId ?? "system", actionType: "REJECT", moduleName: "Goals", newValue: "rejected" });
  ...
  const requestRework = useCallback((id: string, comment: string) => {
    ...
    setState((s) => ({
      ...s,
      goals: s.goals.map((g) => (g.id === id ? { ...g, status: "rework", managerComment: comment } : g)),
    }));
  ...
  const unlockGoal = useCallback((id: string) => {
    // Optimistic update
    setState((s) => ({
      ...s,
      goals: s.goals.map((g) => (g.id === id ? { ...g, isLocked: false, status: "draft" } : g)),
    }));
    pushAudit({ userId: state.currentUserId ?? "system", actionType: "UNLOCK", moduleName: "Goals" });
  ...
  const addCheckIn = useCallback((c: Omit<CheckIn, "id" | "checkinDate">) => {
    ...
    setState((s) => ({
      ...
      goals: s.goals.map((g) => {
        ...
        const merged = { ...g, achievement: c.actualAchievement, updatedAt: new Date().toISOString() };
        const pct = calcProgress(merged);
        return { ...merged, progressPercentage: pct, progressStatus: progressStatusFor(pct) };
      }),
    }));
    pushAudit({ userId: state.currentUserId ?? "system", actionType: "CHECKIN", moduleName: "CheckIns", newValue: `${c.quarter}: ${c.actualAchievement}` });

    // Background server call
    (async () => {
      try {
        const payload = mapCheckInForBackend(checkIn);
        const rawRes = await api.post<any>("/checkins/create", payload);
  ...
  const markAllNotificationsRead = useCallback(() => {
    ...
    setState((s) => ({
      ...s,
      notifications: s.notifications.map((n) => (n.recipientId === s.currentUserId ? { ...n, read: true } : n)),
    }));
