...
function AnalyticsPage() {
  ...
  const exportCSV = () => {
    const headers = ["Employee", "Department", "Goal", "ThrustArea", "Weightage", "Progress%", "Status", "ProgressStatus"];
    const rows = goals.map(g => {
      const u = users.find(x => x.id === g.employeeId);
      return [u?.name ?? "", u?.department ?? "", g.title, g.thrustArea, g.weightage, g.progressPercentage, g.status, g.progressStatus];
    });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-end gap-2 -mb-2">
        ...
        <Button size="sm" onClick={() => window.print()} className="bg-gradient-primary text-primary-foreground hover:opacity-90">
