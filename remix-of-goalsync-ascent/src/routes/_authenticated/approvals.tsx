    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Review goals, leave feedback, or send back for rework. Approved goals lock automatically.
        </p>
      </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-3 max-w-md">
                      ...
                      <label className="text-xs">
                        ...
                        <Input
                          type="number"
                          value={g.weightage}
                          onChange={(e) => updateGoal(g.id, { weightage: Number(e.target.value) })}
                          className="mt-1 h-8"
                        />
                      </label>
                      <label className="text-xs">
                        ...
                        <Input
                          ...
                          value={g.achievement}
                          onChange={(e) =>
                            updateGoal(g.id, { achievement: Number(e.target.value) })
                          }
                          className="mt-1 h-8"
                        />
                      </label>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <Button
                        ...
                        onClick={() => {
                          approveGoal(g.id, comments[g.id]);
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() => {
                          if (!comments[g.id]) return toast.error("Comment required");
                          requestRework(g.id, comments[g.id]);
          ...
          <table className="w-full text-sm">
            <thead className="text-xs text-muted-foreground border-b border-border">
              <tr>
                ...
                <th className="text-left font-medium">Goal</th>
              ...
              {others.map((g) => {
                ...
                return (
                  <tr key={g.id} className="border-b border-border last:border-0">
                    ...
                    <td className="py-2.5">
                      <div className="font-medium truncate max-w-xs">{g.title}</div>
                      <div className="text-[11px] text-muted-foreground">{g.thrustArea}</div>
                    </td>
                    <td className="py-2.5">
                      <StatusBadge status={g.status} />
                    </td>
                    <td className="py-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs">{g.progressPercentage}%</span>
                      </div>
                    </td>
                    <td className="py-2.5 text-right">
                      {g.isLocked && currentUser?.role === "admin" && (
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => {
