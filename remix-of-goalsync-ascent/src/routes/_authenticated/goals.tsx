...
function GoalsPage() {
  ...
  const save = (asDraft: boolean) => {
    ...
    const projectedTotal =
      (editing ? totalWeight - editing.weightage : totalWeight) + form.weightage;
  ...
  const submit = () => {
    if (totalWeight !== 100)
      return toast.error(`Weightage must total 100% (currently ${totalWeight}%)`);
  ...
  const validationOk =
    ...
    myGoals.length > 0 &&
    myGoals.length <= 8 &&
    myGoals.every((g) => g.weightage >= 10);
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Total weightage must equal 100% across up to 8 goals.
          </p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={submit} disabled={!validationOk || drafts === 0}>
        ...
        <div className="flex flex-wrap items-center gap-6">
          ...
          <div className="ml-auto">
            {validationOk ? (
              ...
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-warning/15 text-warning-foreground text-sm font-medium">
                <AlertTriangle className="h-4 w-4" />
                Adjust weightage to 100%
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        {myGoals.length === 0 && (
          <Card className="md:col-span-2 p-12 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Create your first FY26 goal to get started.
            </p>
            <Button
              onClick={openCreate}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select
                  value={form.uomType}
                  onValueChange={(v) => setForm({ ...form, uomType: v as UoMType })}
                >
                  <SelectTrigger className="mt-1.5">
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div>
              ...
              <Input
                ...
                className="mt-1.5"
                placeholder="e.g. Ship payments v2 with 99.9% uptime"
              />
            </div>
            <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setForm({ ...form, achievement: Number(e.target.value) })}
                  className="mt-1.5"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  className={cn("mt-1.5", form.weightage < 10 && "border-destructive")}
                />
              </div>
            </div>
            <div className="text-xs text-muted-foreground">
              Min 10% per goal · current total{" "}
              {totalWeight + (editing ? -editing.weightage : 0) + form.weightage}%
            </div>
          </div>
          <DialogFooter>
