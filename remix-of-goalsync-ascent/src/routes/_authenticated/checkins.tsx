...
function CheckInsPage() {
  ...
  const scope = isManager
    ? goals.filter(
        (g) =>
          users.find((u) => u.id === g.employeeId)?.managerId === currentUser?.id ||
          currentUser?.role === "admin",
      )
    : goals.filter((g) => g.employeeId === currentUser?.id);
  const [quarter, setQuarter] = useState<Quarter>("Q2");
  const [draft, setDraft] = useState<
    Record<string, { actual: number; status: ProgressStatus; comment: string }>
  >({});
      ...
      <div className="flex items-end justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Planned vs Actual — update progress and capture feedback.
          </p>
        </div>
            ...
            <Card key={g.id} className="p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    Target {g.target} · Current {g.achievement}
                  </div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                <label className="text-xs">
                  <span className="text-muted-foreground">Actual achievement</span>
                  <Input
                    type="number"
                    className="mt-1 h-9"
                    value={d.actual}
                    onChange={(e) =>
                      setDraft({ ...draft, [g.id]: { ...d, actual: Number(e.target.value) } })
                    }
                  />
                </label>
                <label className="text-xs">
                  ...
                  <Select
                    value={d.status}
                    onValueChange={(v) =>
                      setDraft({ ...draft, [g.id]: { ...d, status: v as ProgressStatus } })
                    }
                  >
              ...
              <Textarea
                ...
                rows={2}
                value={d.comment}
                onChange={(e) => setDraft({ ...draft, [g.id]: { ...d, comment: e.target.value } })}
              />
              <Button
                size="sm"
              ...
              {previous.length > 0 && (
                <div className="mt-4 border-t pt-3 space-y-1">
                  {previous.map((c) => (
                    ...
                    >
                      <span>
                        <b className="text-foreground">{c.quarter}</b> · planned {c.plannedTarget} /
                        actual {c.actualAchievement}
                      </span>
