...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Sign in to GoalSync to plan and track goals across your team.",
      },
    ],
  }),
  component: LoginPage,
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Failed to sign in. Please verify the backend is running.";
      toast.error(message);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-background">
      <div className="hidden lg:flex relative bg-gradient-primary p-12 flex-col justify-between text-primary-foreground overflow-hidden">
        ...
        <div className="relative space-y-6 max-w-md">
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/15 backdrop-blur text-xs">
            <Sparkles className="h-3 w-3" /> Goal Setting · Check-ins · Analytics
          </div>
          <h1 className="text-4xl font-semibold tracking-tight leading-tight">
            The operating system for enterprise goals.
          </h1>
          <p className="text-primary-foreground/80">
            Align every employee, manager, and HR partner around the same outcomes — from goal
            setting in May to your annual review in April.
          </p>
          <div className="grid grid-cols-3 gap-3 pt-2">
            {[
              ...
              <div key={l} className="rounded-xl bg-white/10 backdrop-blur p-3">
        ...
        <div className="relative text-xs opacity-80">
          © {new Date().getFullYear()} GoalSync. SOC 2 Type II · ISO 27001
        </div>
      </div>
      ...
      <div className="flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-md">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Use a demo persona below or your credentials.
          </p>
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            ...
            <Button
              ...
              onClick={() => toast.info("Microsoft Entra ID SSO would launch here.")}
            >
            ...
            <div className="space-y-1">
              {PERSONAS.map((p) => (
                <button
                  ...
                  className="w-full flex items-center justify-between text-left px-2 py-1.5 rounded-md hover:bg-card"
                >
                  <div>
